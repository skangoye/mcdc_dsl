...
//		System.out.println()
	...
	def private static void toBTExpression(EXPRESSION expression, BinaryTree<Couple<String,String>> bt){
		switch(expression){
			OR: {	
				...
				//val leftTree = bt.left
				//val rightTree = bt.right
				//leftTree.setParent(bt)
				//rightTree.setParent(bt)
			...
			enumConstant: { 
				val comparator = expression.getContainerOfType(EQUAL_DIFF)
	  			...
	  			bt.setTree(new BinaryTree<Couple<String,String>>(new Couple(enumVarName_Plus_EnumValue,  "c_enum")))
	  		}
			
			//bt.setTree(new BinaryTree< Couple<String,String> >(new Couple(expression.value.toString,"c_enum")))
			
			intConstant: bt.setTree(new BinaryTree< Couple<String,String> >(new Couple(expression.value.toString,"c_int")))
	  		
	  		realConstant: bt.setTree(new BinaryTree< Couple<String,String> >(new Couple(expression.value.toString,"c_real")))
	  		
	  		strConstant: bt.setTree(new BinaryTree< Couple<String,String> >(new Couple(expression.value.toString,"c_str")))
	  		
	  		boolConstant: bt.setTree(new BinaryTree< Couple<String,String> >(new Couple(expression.value.toString,"c_bool")))
	  		
	  		bitConstant: bt.setTree(new BinaryTree< Couple<String,String> >(new Couple(expression.value.toString,"c_bit")))
	  		
	  		hexConstant: bt.setTree(new BinaryTree< Couple<String,String> >(new Couple(expression.value.toString,"c_hex")))
		}
	}//toBTExpression
	...
	/**
	 * Search in the Binary Tree Expression, a variable with name 'varName' and replace it with its corresponding value holding in assign couple. assign couple is an 
	 * element of the 'assignList'
	 * 'varName is the first parameter of the couple assignCouple', the second parameter of the couple is the expression value (expressed over the symbolic values) of varName 
	 */
	...
	def static pathToPathCondition(MODULE_DECL module, List<Triplet<List<String>, List<String>, List<String>>> pathSequence, MCDC_Statement mcdcStatement){
		...
		val pathConditionList = new ArrayList<BinaryTree<Couple<String,String>>>
		val assignmentList = new ArrayList<Couple<String, BinaryTree<Couple<String,String>>>> 
		
		val listOfVariables = module.interface.declaration //module's interface variables 
		...
		listOfVariables.forEach[ 
			variable | switch (variable){
							...
							CST_DECL:{ //constant  
								...
								val cstType = "c_" + variable.type.type
	...
	/*def static parentToString(BinaryTree<Couple<String,String>> bt){
		val parent = bt.getParent
		if(parent != null)
			return parent.value.first.toLowerCase
		else
			return "null"
	}*/
